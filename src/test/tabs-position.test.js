...
suite("tabs positions and indexes", () => {
  ...
  function assertDOMOrderCorrect(ffTabs) {
    ...
    const domTabsIds = [...document.querySelectorAll(".tab")].map(e =>
      parseInt(e.getAttribute("data-tab-id")),
    );
  ...
  test("attach from other window", async () => {
    ...
    await assertOrderAndIndexes();
    const { id: otherTabId1 } = (await browser.tabs.query({
      windowId: otherWindowId,
    }))[0];
    const { id: otherTabId2 } = await browser.tabs.create({
      windowId: otherWindowId,
    });
    const { id: otherTabId3 } = await browser.tabs.create({
      windowId: otherWindowId,
    });
